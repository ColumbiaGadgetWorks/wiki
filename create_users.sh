...
CREATE USER IF NOT EXISTS '$WIKI_SCHEMA_USER'@'%' IDENTIFIED BY '$WIKI_SCHEMA_PASSWORD';
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, DROP, INDEX ON $MYSQL_DATABASE.* TO '$WIKI_SCHEMA_USER'@'%';

CREATE USER IF NOT EXISTS '$WIKI_APP_USER'@'%' IDENTIFIED BY '$WIKI_APP_PASSWORD';
GRANT SELECT, INSERT, UPDATE, DELETE ON $MYSQL_DATABASE.* TO '$WIKI_APP_USER'@'%';
EOF
fi
